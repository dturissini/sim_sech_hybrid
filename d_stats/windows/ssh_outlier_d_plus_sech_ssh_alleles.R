...
pdf_file <- paste('ssh_outlier_d_plus_sech_ssh_alleles_', win_size, '.pdf', sep='')
...
win_site_table <- paste('outlier_d_plus_win_sites_', win_size, sep='')
allele_table_sech <- paste('outlier_d_plus_win_sechellia_alleles_', win_size, sep='')
allele_table_ssh <- paste('outlier_d_plus_win_sim_sech_hybrid_alleles_', win_size, sep='')
...
for (dsw_id in sort(unique(win_sites$dsw_id)))
  {
  cat(dsw_id, as.character(Sys.time()), "\n")
  #ssh freqs
  par(mar=c(0, 10.1, 4.1, 1.1))
  plot(win_sites$pos[win_sites$dsw_id == dsw_id], win_sites$der_alleles_sech[win_sites$dsw_id == dsw_id] / win_sites$total_alleles_sech[win_sites$dsw_id == dsw_id], pch=20, col='red', xaxt='n', ylim=c(0,1), ylab='Der sech freq', main='')
  abline(v=gwas_pos$pos[gwas_pos$dsw_id == dsw_id], col='black')

  mtext(paste('D+ = ', round(d_wins$d_plus[d_wins$dsw_id == dsw_id], 2), sep=''), 0, cex=.7, side=3, line=2, at=min(win_sites$pos[win_sites$dsw_id == dsw_id]) - .18 * (max(win_sites$pos[win_sites$dsw_id == dsw_id]) - min(win_sites$pos[win_sites$dsw_id == dsw_id])), adj=0)
  mtext(paste(round(sum(d_wins$d_plus[d_wins$dsw_id == dsw_id] > d_wins$d_plus) / length(d_wins$d_plus), 3), ' percentile', sep=''), 0, cex=.7, side=3, line=1, at=min(win_sites$pos[win_sites$dsw_id == dsw_id]) - .18 * (max(win_sites$pos[win_sites$dsw_id == dsw_id]) - min(win_sites$pos[win_sites$dsw_id == dsw_id])), adj=0)
  ...
  alleles <- dbGetQuery(conn, paste("select a.pos, a.sample_id, a.num_der_alleles, abs(a2.num_der_alleles - a.num_der_alleles) num_der_alleles_adj
                                     ...
                                     where a.dsw_id = ", dsw_id, "
                                     and a.dsw_id = a2.dsw_id
                                     and a.pos = a2.pos
  ...
  plot(1, type='n', xlim=range(win_sites$pos[win_sites$dsw_id == dsw_id]), ylim=c(0, nrow(samples)), xlab=paste('Pos on', d_wins$chrom[d_wins$dsw_id == dsw_id]), ylab='', main='', yaxt='n')
  ...
  if (sum(genes$dsw_id == dsw_id) > 0)
    {
    rect(genes$start[genes$dsw_id == dsw_id], -nrow(samples), genes$end[genes$dsw_id == dsw_id], 1.5*nrow(samples), col=adjustcolor('gray', .2), border=NA)
    }  
