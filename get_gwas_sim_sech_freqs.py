...
#python3 /proj/matutelb/projects/gwas/scripts/get_gwas_sim_sech_freqs.py /proj/matutelb/data_share/simulans_OA_resistance/simulans_sechellia.chroms.biallelic.filtered.recode.OA.filtered.recode.vcf.gz /proj/matutelb/projects/gwas/genotype_datasets/sech_oa/sim_sech_final_samplelist.tsv /proj/matutelb/projects/gwas/gwas_results/sech_oa_only_sim_pca/results/sech_oa_only_sim_pca_gwas.oa_resistance.glm.linear 1e-5 /proj/matutelb/projects/gwas/gwas_results/sech_oa_only_sim_pca/results/sech_oa_sim_only_pca_gwas_allele_freqs.db
...
def main():
  ...
  cur.execute("drop table if exists gwas_sim_sech_freqs")
  cur.execute("""create table gwas_sim_sech_freqs
  ...
  cur.execute("create index idx_gssf_pos on gwas_sim_sech_freqs(pos)")
...
#process gwas file
  gwas_snps = {}
  with open(gwas_file, 'r') as l: 
    for line in l:
      if line[0] != '#':
        line = line.strip()
        values = line.split('\t')
        if values[10] not in ['NA']:
          if float(values[10]) < p_cutoff:
            snp_name = values[0] + '_' + values[1]
            gwas_snps[snp_name] = values[10]
    ...
    with gzip.open(vcf_file, 'rt') as v: 
      for line in v:
        ...
        if line[:2] != "##":
          ...
          if line[0] == '#':
            fly_lines = values[9:]
          elif values[0] in lk_chrom and snp_name in gwas_snps:
            ...
            if num_samples['sim_sech_hybrid'] > 0: 
              ...
              gssf_id += 1        
              o.write(f"{gssf_id}\t{lk_chrom[values[0]]}\t{values[1]}\t{gwas_snps[snp_name]}\t{values[3]}\t{values[4]}\t{num_samples['simulans']}\t{num_samples['sechellia']}\t{num_samples['sech_denis']}\t{num_samples['sim_sech_hybrid']}\t{sim_alt_freq}\t{sech_alt_freq}\t{sech_denis_alt_freq}\t{sim_sech_hybrid_alt_freq}\n")                  
    
    o.flush() 
    
    os.system(f"""sqlite3 {db_file} ".mode tabs" ".import {o.name} gwas_sim_sech_freqs" """)


  conn = sqlite3.connect(db_file)    
  cur = conn.cursor()                            

  cur.execute("""update gwas_sim_sech_freqs 
