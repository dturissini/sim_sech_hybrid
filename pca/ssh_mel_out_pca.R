...
#all
all_pops <- dbGetQuery(conn, "select s.sample_id, l.pop, col
                              from sample_species s, sample_pop_link l, pop_cols c
                              where s.sample_id = l.sample_id
                              and l.pop in ('sim', 'sech', 'ssh', 'mel')
                              and l.pop = c.pop")
                              
pca_plots('sim_sech_outgroup_mel_pca', all_pops)
pca_plots('sim_sech_outgroup_mel_all_sites_pca', all_pops)



#sech pops
sech_pops <- dbGetQuery(conn, "select s.sample_id, l.pop, col
                              from sample_species s, sample_pop_link l, pop_cols c
                              where s.sample_id = l.sample_id
                              and l.pop in ('sechden', 'sechpras', 'sechanro', 'sechlad', 'sechunk', 'sechmari')
                              and l.pop = c.pop")

pca_plots('sech_pca', sech_pops)
pca_plots('sech_all_sites_pca', sech_pops)


#sechbase pops
sechbase_pops <- dbGetQuery(conn, "select s.sample_id, l.pop, col
                                   from sample_species s, sample_pop_link l, pop_cols c
                                   where s.sample_id = l.sample_id
                                   and l.pop in ('sechanro', 'sechlad', 'sechmari')
                                   and l.pop = c.pop")
...
#sech_ssh
sech_ssh_pops <- dbGetQuery(conn, "select s.sample_id, l.pop, col
                                   from sample_species s, sample_pop_link l, pop_cols c
                                   where s.sample_id = l.sample_id
                                   and l.pop in ('sshlad', 'sshmahe', 'sshanro', 'sechden', 'sechpras', 'sechanro', 'sechlad', 'sechunk', 'sechmari')
                                   and l.pop = c.pop")
