...
#python3 /proj/matutelb/projects/gwas/scripts/get_sim_sech_diff_sites.py /proj/matutelb/data_share/simulans_OA_resistance/simulans_sechellia.chroms.biallelic.filtered.recode.OA.filtered.recode.vcf.gz /proj/matutelb/projects/gwas/genotype_datasets/sech_oa/sim_sech_final_samplelist.tsv /proj/matutelb/projects/gwas/genotype_datasets/sech_oa/sim_sech_diff_sites.db
...
def main():
  ...
  cur.execute("drop table if exists sim_sech_diff_sites")
  cur.execute("""create table sim_sech_diff_sites
                 (ssds_id int primary key,
                  chrom varchar(20),
                  pos int,
  ...
  cur.execute("create index idx_ssds_pos on sim_sech_diff_sites(pos)")
      ...
      for line in v:
        ...
        if line[:1] != "##":
          ...
          if line[0] == '#':
            ...
            if num_samples['simulans'] > 0 and num_samples['sechellia'] > 0 and num_samples['sech_denis'] > 0:  
              ...
              if abs(sim_alt_freq - sech_denis_alt_freq) == 1 or abs(sim_alt_freq - sech_denis_alt_freq) == 1:  
                ssds_id += 1        
                o.write(f"{ssds_id}\t{lk_chrom[values[0]]}\t{values[1]}\t{values[3]}\t{values[4]}\t{num_samples['simulans']}\t{num_samples['sechellia']}\t{num_samples['sech_denis']}\t{sim_alt_freq}\t{sech_alt_freq}\t{sech_denis_alt_freq}\n")                  
    
    o.flush() 
    
    os.system(f"""sqlite3 {db_file} ".mode tabs" ".import {o.name} sim_sech_diff_sites" """)
