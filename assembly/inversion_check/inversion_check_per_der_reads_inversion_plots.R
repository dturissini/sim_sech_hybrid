...
for (sample_id in sample_ids$sample_id)
  ...
  for (i in 1:nrow(possible_inversions))
    ...
    if (substr(possible_inversions$name[i], 1, 4) == 'rand')
      {win_site_table <- paste('outlier_random_win_sites_', win_size, '_sim_ssh_sech_mel', sep='')}
    ...
    if (nrow(read_alleles) > 0)
      {
      freq_win_size <- 2000
      starts <- seq(possible_inversions$start[i], possible_inversions$end[i] + freq_win_size, freq_win_size)
      avg_win_der_freqs <- c()
      avg_win_anro_freqs <- c()
      for (start in starts)
        {
        win_filter <- read_alleles$pos >= start & read_alleles$pos < start + freq_win_size
      ...
      points(starts + freq_win_size / 2, avg_win_der_freqs, type='l', col='blue')
      ...
      points(starts + freq_win_size / 2, avg_win_anro_freqs, type='l', col='red')
